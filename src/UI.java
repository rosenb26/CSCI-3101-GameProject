...
public class UI {
    ...
    public void exitFinder() {
        ...
        if (cellDistance > this.encounters.size() / 2) {
            cellDistance = this.encounters.size() - cellDistance;
            flag = true;
        ...
        if (flag) {
            if (this.playerPosition < exitPosition) {
    ...
    public void nearbyCellViewer() {
        System.out.print("How many nearby cells (on each side of you) would you like to see? ");

        while (true) {

            try {
                System.out.println("Enter a positive integer or press 'x' to return to the Main Menu: ");
                String input = this.reader.nextLine();
                if (input.equalsIgnoreCase("x")) {
                    this.printMainMenu();
                    return;
                }
                int neighbors = Integer.valueOf(input);
                if (neighbors > 0) {
        ...
        if (name.equals("Encounter")) {
            ...
            if (name.equals("Enemy")) {
                ...
                if (this.monsterDefeated) {
                    ...
                    this.playerPosition = this.dungeon.getPlayerPosition();
                    return;
                }
            }
        ...
        while (true) {
            ...
            try {
                ...
                if (choice == 1) {
                    ...
                    this.performCombatMath(monster, this.encounters.get(this.playerPosition), cell);
                    break;
                }
                else if (choice == 3) {
    ...
    public void performCombatMath(Encounter first, Encounter second, int cell) {
        ...
        if (defender.getLife() <= 0) {
            if (defender.getName().equals(this.encounters.get(this.playerPosition).getName())) {
                ...
                int experienceBefore = attacker.getExperience();
                attacker.setExperience(attacker.getExperience() + monsterExperience);
                int experienceAfter = attacker.getExperience();
                if (experienceAfter % 100 <= experienceBefore % 100) {
                    attacker.levelUp();
                }
                Player player = (Player) this.encounters.get(this.playerPosition);
